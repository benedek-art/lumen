...
final class RobustnessTests: XCTestCase {
    ...
    // MARK: - The coherence window has to match the buffer

    /// Negative Texture's gate stops telling an edge from texture if the structure
    /// tensor is smoothed over a window that is a large fraction of the frame.
    ///
    /// This is the reference half of a GPU failure. `RenderGraph.applyPresence` sizes
    /// the coherence window off the render's `longEdge`, and the golden handed it 1600
    /// with a 64 px frame, so the window came out at 8 where the reference uses 1 on
    /// that buffer. The GPU then moved the edge by 0.04258 and the texture by 0.01364
    /// — backwards — and read as a broken gate. It was not: the REFERENCE does the same
    /// thing at the same window, which is what this pins.
    ///
    /// So the property is not "the gate works", it is "the gate works while the window
    /// tracks the buffer, and stops when it does not". Both halves are asserted, because
    /// the first alone would pass on a clamp that silently disabled the gate.
    func testTheCoherenceWindowHasToTrackTheBuffer() {
        ...
        let source = ImageBuffer(width: width, height: height) { u, v in
            ...
            if v < 0.5 {
                ...
                value = 0.13 + 0.08 * v          // a cheek, a clear sky
            }
            return RGB(gray: value)
        }

        func movement(workingRadius: Int) -> (texture: Double, edge: Double) {
            let d = DetailEngine.Decomposition(image: source, workingRadius: workingRadius)
            let out = DetailEngine.applyTexture(source, amount: -100, decomposition: d)
            func mean(xs: Range<Int>, ys: Range<Int>) -> Double {
                var total = 0.0, count = 0
                for y in ys {
                    for x in xs {
                        total += abs(out[x, y].g - source[x, y].g)
                            / Swift.max(source[x, y].g, 1e-6)
                        count += 1
                    }
                }
                return count > 0 ? total / Double(count) : 0
            }
            return (mean(xs: 4..<20, ys: 4..<28), mean(xs: 30..<34, ys: 4..<28))
        }

        // The window this buffer earns: `max(Int(64 * 0.02), 3) / 4` floored at 1 = 1.
        let matched = movement(workingRadius: 4)
        XCTAssertGreaterThan(matched.texture, 1e-5,
                             "negative Texture smoothed nothing, so this proves nothing")
        XCTAssertGreaterThan(matched.texture, matched.edge,
                             "at the window this buffer earns, negative Texture moved "
                                 + "the edge by \(matched.edge) and the texture by "
                                 + "\(matched.texture) — the gate is not distinguishing "
                                 + "them")

        // Eight times too wide, which is what an unclamped `longEdge` of 1600 asks for.
        let oversized = movement(workingRadius: 32)
        XCTAssertGreaterThan(oversized.edge, oversized.texture,
                             "an 8x oversized coherence window no longer inverts the "
                                 + "ordering (edge \(oversized.edge), texture "
                                 + "\(oversized.texture)) — if the reference has changed "
                                 + "so that the window no longer matters, "
                                 + "RenderGraph.structureWindow's clamp is now "
                                 + "load-bearing for a reason this test no longer states")
    }
}
