...
final class RecipeCodecToleranceTests: XCTestCase {
    ...
    private static func populated() -> Recipe {
        ...
        let develop = Develop(
            raw: RawParams(decoder: "lumen", decoderVersion: 7, temp: 5600, tint: 12),
            tone: Tone(exposure: 0.4, contrast: 15, contrastPivot: -0.5, highlights: -30,
                       shadows: 25, whites: 10, blacks: -12),
            zones: Zones(pivots: [0.07, 0.24, 0.49, 0.74, 0.91],
                         dark: zone(0.3), shadow: zone(-0.2), mid: zone(0.1),
                         light: zone(-0.4), bright: zone(0.6), global: zone(0.05)),
            curve: curve,
            color: ColorAdjust(vibrance: 15, saturation: 25, density: 60, protectSkin: 40),
            mixer: Mixer(bands: (0..<8).map { index in
                MixerBand(hue: Double(index) + 1, sat: Double(index) - 4,
                          lum: Double(index) * 2,
                          core: [20 + Double(index), 25 + Double(index)],
                          feather: [12 + Double(index), 18 + Double(index)])
            }, uniformity: 30),
            pointColors: [swatch],
