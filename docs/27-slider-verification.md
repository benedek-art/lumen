...
| `mixer.uniformity` | The mixer's 25th slider — and the audit's biggest catch: the contract probe convicted the shipped convergence field itself. Each band applied a full-deviation pull toward its own centre, so at seams two opposing pulls cancelled into small BACKWARDS moves — hues ~20° from a centre anti-converged, and wheel-wide aggregate convergence at uniformity 100 measured +0.1° on 54° of pair spread. Fixed before the first record pinned it: convergence now pulls toward the weighted circular mean of the member bands' targets, a monotone field with no anti-pockets. The measured `bandMeanHues` wiring is now IN: `PipelineRenderer.measuredBandMeanHues` measures each file once off a ~512 px neutral decode and threads the result through `RenderPlan(bandMeanHues:)` into the engine and the colour-grade table's cache key on every rendering path, so Uniformity converges on the image's own measured hues (fallback to the core-arc midpoint only when the frame measures as grey — nil, never an invented number). Known limitation, accepted and recorded: the basis is the NEUTRAL rendition, so the target holds still while editing — a strong user WB change shifts the picture's hues off the measured basis and the target lags by that shift; still the image's own hues, no longer exactly its current ones. What still remains: the wide band feathers leave mid-band ramps near identity, so convergence is strong only in the ±6° flats around each centre — that half rides the dossier's band-geometry item. Contract: 8° pairs at every centre converge >70%; every seam hue holds within 2.5°. |
