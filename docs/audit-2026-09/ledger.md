...
| C2-03 / K-065 | C2 | S2 | **(this batch)** | the export path grains whichever grain the plan resolved, after the resize — it read `plan.filmChain`, so a creative grain fell through to the graph and was laid pre-resize, past the half-pixel floor. `RenderGraph.defersGrain` is now told rather than inferred from a missing plate |
| N-003 | C2 | S2 | **(this batch)** | FOUND WHILE LANDING: C2-01b never reached the film path's GPU plate — `PipelineRenderer` held a second CIImage plate builder that asked for the unlimited plate. One builder now; `testThereIsOneGPUPlateBuilderAndItAsksThePlanForItsScale` scans for a third |
| N-004 | C2 | S3 | **OPEN** | FOUND WHILE MEASURING: `plateSeed(channel:)`'s three "independent" fields correlate at r ≈ 0.088 / 0.044 / −0.040 over 16 384 samples. Makes C2-02 smaller, not larger; it is why the chroma mix preserves amplitude to a few percent rather than exactly. Plate-generator question, W3 |
| C2-02 | C2 | S1 | **(this batch)** | `FilmGrainProfile.chroma` — how much of the three dye layers' independence reaches the picture. χ = 0.30 colour negative, 0.15 reversal, 1 (identity) monochrome and creative. Measured R−G/luma 2.06 → 0.41, B−G 2.28 → 0.45. χ = 1 is bit-for-bit what shipped |
