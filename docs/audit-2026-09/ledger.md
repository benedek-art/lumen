...
| N-002 | I1 | S2 | `w0/perf-baseline.md` DRAGPROBE | Settle path bakes tables on every frame with zero cache hits for Whites (`0h/36b`, p50 388 ms vs 69 ms draft) and Saturation (p50 269 ms vs 24 ms draft) — the settle frame after a drag costs 7–11× the drafts that preceded it. | PARTLY REJECTED by I1: the `0h` is a probe artifact (disjoint sample sets + `resetStats` not clearing entries); the `36b` is real and is a double/triple bake | **LANDED cf9fb58** (I1-01 + I1-03): `table` joins an in-flight bake for its key or steals a queued one; `drainPending` counts `deferredBakes`; `resetStats` zeroes `slotTraffic`. I1-04 landed **abab18a** (the settle loop's retry gate). **RE-MEASURED at `1df319f` (w6/perf-final.md) and the row is UNCHANGED at `0h/36b/0s`** — which is not evidence the fixes failed, because I1-02 (the probe's disjoint sample sets and its un-cleared `resetStats`) never landed, so this column cannot distinguish that from a probe that cannot see them. Saturation's settle DID move, 24h/12b → 25h/11b: one bake became a hit, the only direct evidence in the table that the join works. **CLOSED at `db43c41`** — with the probe corrected (I1-02: each settle sample renders its own draft frame first, because the 8-entry slot means a drag's tables are gone by the time a spread settle asks for them), Whites' settle reads **`34h/2b/0s` against `0h/36b/0s`**, and every other settle row is `36h/0b`. The settle path was always wired to the cache; the probe could not see it. W2's verdict — "the `0h` is a probe artifact" — was right. |
...
| I1-04 | I1 | S2 | **abab18a** | the settle loop's retry gate takes the PHOTOGRAPH (`anyBakePending(for:)`, asked of `pending` and of a new `inFlightIdentity`) and only fires for a DRAFT frame — a settle cannot ride a stale table, and another photograph's bake cannot change this one's pixels |
| C2-01b | C2 | S2 | **0158776** | band-limited plate: octaves finer than 2 render pixels are skipped, so the preview is the low-pass of the export. `film.grain.size` re-measured (authority 40.689 → 40.130) |
| C2-03 / K-065 | C2 | S2 | **6425f60** | the export path grains whichever grain the plan resolved, after the resize — it read `plan.filmChain`, so a creative grain fell through to the graph and was laid pre-resize, past the half-pixel floor. `RenderGraph.defersGrain` is now told rather than inferred from a missing plate |
| C2-05 | C2 | S2 | **(this batch)** | `FilmLab.halationSize` (0.5–2.0, nil = the emulsion's) and `.halationRedness` (0–100, nil = the stock's own) reach the wire, the tolerant decoder and two `LookPanel` rows. `HalationProfile` has computed from both since it was written and both callers passed the defaults. Landed at the accessor BOTH renderers call, not the unused `halationProfile`. Defaults are byte-identical, so no proof record moves. Both optional so a present `filmLab` gains no keys |
| C2-06 | C2 | S3 | **(this batch)** | `CreativeGrain`'s header promised "three decorrelated layers on a colour picture"; the code has built ONE luminance field since the owner rejected the rainbow, and `size` said 4…32 µm where the code is 7…56 |
| C2-07 | C2 | S3 | **(this batch)** | the Effects stock grain rows were keyed `look.grain.*` — the creative rows' own strings, for different fields, and different from the keys `LookPanel` binds those very fields under. Now `film.grain.*`, matching `LookPanel`; a text scan holds both halves |
| C2-04 | C2 | S2 | **OPEN** | the test that owns the half-pixel floor asserts the divergence rather than the parity — revisit in W3 now that C2-01b has changed what it should say |
| N-006 | C2 | S3 | **OPEN** | FOUND WHILE LANDING C2-05: `HalationProfile.normalizedWeights` exists and NEITHER renderer uses it — both apply the raw bounce weights 1 / 0.5 / 0.25 summing to 1.75, so Halation Amount is scaled by 1.75. They agree, so it is a calibration question and not a parity break; normalising changes halation pixels and needs its own ceremony |
| N-004 | C2 | S2 | **6425f60** | FOUND WHILE LANDING: C2-01b never reached the film path's GPU plate — `PipelineRenderer` held a second CIImage plate builder that asked for the unlimited plate. One builder now; `testThereIsOneGPUPlateBuilderAndItAsksThePlanForItsScale` scans for a third |
| N-005 | C2 | S3 | **OPEN** | FOUND WHILE MEASURING: `plateSeed(channel:)`'s three "independent" fields correlate at r ≈ 0.088 / 0.044 / −0.040 over 16 384 samples. Makes C2-02 smaller, not larger; it is why the chroma mix preserves amplitude to a few percent rather than exactly. Plate-generator question, W3 |
| C2-02 | C2 | S1 | **6425f60** | `FilmGrainProfile.chroma` — how much of the three dye layers' independence reaches the picture. χ = 0.30 colour negative, 0.15 reversal, 1 (identity) monochrome and creative. Measured R−G/luma 2.06 → 0.41, B−G 2.28 → 0.45. χ = 1 is bit-for-bit what shipped |
