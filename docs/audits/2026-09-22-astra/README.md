...
**Subsequent authorized implementation:** the owner has now requested a full repair programme. Follow the [repair plan](REPAIR-PLAN.md), [live finding ledger](repair-ledger.json), [initial execution record](EXECUTION-01.md), [ownership/recovery follow-up](EXECUTION-02.md), [integrated RAW/mask/cache/control/export work](EXECUTION-03.md), and [open colour-table investigation](EXECUTION-04-colour-lut.md). Historical audit results below remain unchanged; repair status is recorded separately. Work is isolated on `codex/lumen-verified-repairs`, not installed or released.
