...
The published safety checkpoint `40e1048` passed every required main CI job plus the separate GPU and UI-layout workflows. At checkpoint `9836d68`, Linux engine/fixtures, macOS build/app bundle and separate GPU/UI-layout jobs passed; longer test/proof workflows were still running when this note was written. These results do not certify newer unpushed commits.
