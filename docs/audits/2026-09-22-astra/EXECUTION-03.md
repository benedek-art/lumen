...
The third combined optimized run at **`6c9c026` passed 2,538 tests with 14 skips and zero unexpected failures**, including all five private RAW test methods against the three copied Sony files. Test time was 184.343 seconds; build plus test wall time 299.067 seconds. Command: `LUMEN_AUDIT_RAW_DIR=<private-copies> swift test --package-path work/lumen --scratch-path <isolated-build> --build-system native -c release --jobs 2`. Logs remain private. The final Swift source-surface checker also passed. Its Float16 known-symbol entry was added only after actual native compilation established the symbol; this did not relax numerical checks.

**The full run accepted 36 expected assertions, not just the four colour assertions:** ColorTableAccuracy 4, CurveAdversarial 8, IngestAdversarial 16, LayoutMetric 3, MaskDependencyAdversarial 2 and RollCursorAdversarial 3. The 32 inherited assertions were already documented in the original audit's detailed test results; they remain unfinished work, not new fixes or 32 newly discovered independent bugs. Earlier compact checkpoint summaries that mentioned only colour/layout were incomplete inventories. [Supplemental tracking](SUPPLEMENTAL-BACKLOG.md) records severity/contract distinctions and ownership. In particular, sole-file preservation and honest ingest/ejection reporting need further repair before relying on this as a sole library.

The 14 skips are 11 external RAW-corpus tests (manifest unset), two opt-in control-probe methods, and one opt-in CPU benchmark. Private RAW tests did run; the wider camera corpus and benchmark did not. Neither expected failures nor skips certify those behaviours.
